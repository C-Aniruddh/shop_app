...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void setUserData(String type, String uid) async {
    ...
    if (type == "user"){
      ...
      await Firestore.instance.collection('users')
        ...
        Firestore.instance.collection('users').document(docs.documents[0].documentID)
      ...
      Firestore.instance.collection('shops')
          ...
          .then((docs){
        userData = docs.documents[0];
        Firestore.instance.collection('shops').document(docs.documents[0].documentID)
            .updateData({'token': token});
        setState(() {
          userData = docs.documents[0];
          userLoaded = true;
    ...
    await FirebaseAuth.instance.currentUser().then((user){
      if(user == null){
        ...
        Firestore.instance.collection('uid_type')
          ...
          .getDocuments()
          .then((docs) async {
            type = docs.documents[0]['type'];
            setState(() {
              print(docs.documents[0]['type']);
               user_type = docs.documents[0]['type'];
               user_uid = user.uid;
               uid = user.uid;
            });
            await setUserData(docs.documents[0]['type'], user.uid);
        });
      }
    });
  ...
  String distanceBetween(String shop_geohash){
    ...
    double meter = distance(new LatLng(shop_coordinates[1],shop_coordinates[0]),
  ...
  Widget buildHomeUser(){
    ...
    return Container(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: Firestore.instance.collection('shops')
                      ...
                      child: Padding(
                        ...
                        child: new ListTile(
                          ...
                          leading: CircleAvatar(
                            ...
                            child: Text(document['shop_name'][0].toString().toUpperCase()),
                          ),
                          title: Text(document['shop_name']),
                          subtitle: Text(distanceBetween(document['shop_geohash']) + " meters away",),
                          trailing: IconButton(icon: Icon(Icons.map), onPressed: (){
                            print("Open");
                            MapUtils.openMap(document['shop_lat'], document['shop_lon']);
                          },),
                        ),
                      ),
                    );
        ...
        builder: (context) => StatefulBuilder(
          builder: (BuildContext context, StateSetter setStateSheet) =>
              SingleChildScrollView(
                child: Container(color: Colors.grey[900], height:MediaQuery.of(context).size.height* 0.5,
                    child: Column(
                      children: <Widget>[
                        Align(
                                            ...
                                            children: <Widget>[
                                              ...
                                              Card(child: Padding(
                                                  ...
                                                  child: ListTile(
                                                    ...
                                                    subtitle: Text(document['appointment_start'].toString()),
                                                  )
                                              )),
                                              Card(child: Padding(
                                                  padding: const EdgeInsets.all(8.0),
                                                  child: ListTile(
                                                    ...
                                                    title: Text("End Time"),
                                                    subtitle: Text(document['appointment_end'].toString()),
                                                  )
                                              )),
                  ...
                  children: snapshot.data.documents
                    ...
                    return Card(
                      ...
                      child: Padding(
                        ...
                        child: new ListTile(
                          onTap: (){
                            _showCompleteDialog(context, document.documentID, document['otp']);
                            //Navigator.push(context, MaterialPageRoute(builder: (context)=> ShopPage(shopDetails: document, userDetails: userData,)));
                          },
                          leading: CircleAvatar(
                            backgroundColor: Colors.cyan,
                            child: Text(document['shopper_name'][0].toString().toUpperCase()),
                          ),
                                                            ...
                                                            child: Column(
                                                              children: <Widget>[
                                                                Card(child: Padding(
                                                                  ...
                                                                  child: ListTile(
                                                                    title: Text("User needs : "),
                                                                    subtitle: Text(document['items'], overflow: TextOverflow.ellipsis, maxLines: 5,),
                                                                  ),
                                                                )),
                                                                InkWell(
                                                                  onTap: (){
                                                                    DatePicker.showTime12hPicker(context,
                                                                        theme: DatePickerTheme(backgroundColor: Colors.grey[900], itemStyle: TextStyle(color: Colors.white),
                                                                    ...
                                                                    child: PhoneAuthWidgets.textFieldDisabled(startTimeController)),
                                                                InkWell(
                                                                  onTap: (){
                                                                    DatePicker.showTime12hPicker(context,
                                                                        theme: DatePickerTheme(backgroundColor: Colors.grey[900], itemStyle: TextStyle(color: Colors.white),
                                                                    ...
                                                                    child: PhoneAuthWidgets.textFieldDisabled(endTimeController)),
                                                                Center(
                                                                  child: RaisedButton(
                                                                    color: Colors.cyan,
                                                                    child: Text("Confirm"),
                                                                  onPressed: (){
                                                                      Firestore.instance.collection('appointments')
                                                                          ...
                                                                          .updateData({'appointment_start': startTime, 'appointment_end': endTime,
